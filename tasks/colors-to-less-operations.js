...
module.exports = function(grunt) {
  ...
  function changeWithVar(file, col1, col2, line) {
    lineReader.eachLine(file, function(line2, last) {
      if (line2.indexOf(col1 + ":") != -1) {
        ...
        var arr2 = line.split(' ');
        var changeTo = arr2[0] + " " + lessString;              // final string with less functions

        var arr3 = line.split(/[()]/);
        var changeThat = arr3[0] + "\\(" + arr3[1] + "\\)" + arr3[2];  // string which I'll replace (it's regexp)
  ...
  function changeWithoutVar(file, col1, col2, line) {
    var lessString = transformToLessOperations(col1, col2);  // get less function with my colors
    var arr = line.split(' ');
    var changeTo = arr[0] + " " + lessString;              // final string with less functions
    var arr2 = line.split(/[()]/);
    var changeThat = arr2[0] + "\\(" + arr2[1] + "\\)" + arr2[2];  // string which I'll replace (it's regexp)
          ...
          if(line.indexOf(funcName) != -1) {              // find line with function name
            ...
            for(; i < color1.length; i++) {
              if(color1[i].indexOf("@") != -1) {         // if using less variable (like @green)
                num.push(i);
                changeWithVar(file, color1[num[0]], color2[num[0]], lines[num[0]]);  // find hex value of less variable
                num.shift();
              }
              else {
                num2.push(i);
                changeWithoutVar(file, color1[num2[0]], color2[num2[0]], lines[num2[0]]);
                num2.shift();
              }
            }
          }
